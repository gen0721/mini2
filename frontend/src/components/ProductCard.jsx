...
export default function ProductCard({ product, style={} }) {
  ...
  return (
    ...
    >
      {/* ElectricBorder только для promoted или при hover — иначе рендерим обычную карточку */}
      {showElectric ? (
        <ElectricBorder
          color={electricColor}
          ...
          borderRadius={20}
        >
          <CardInner product={product} img={img} seller={seller} />
        </ElectricBorder>
      ) : (
        <CardInner product={product} img={img} seller={seller} />
      )}
...
function CardInner({ product, img, seller }) {
  return (
